...
\end{align*}
By constraining the possible regions we consider, we can now define our test by:
\begin{align*}
	\text{reject } H_0 \text{ if } \|\sqrt{n} \hat{\boldsymbol{\psi}}\|_2 \geq r_\alpha.
\end{align*}
Similarly, p-values can now be defined as $Pr_Q(\|\rvv\|_2 \geq \|\sqrt{n} \hat{\boldsymbol{\psi}}\|_2)$.
...
Above, a norm mapped the distribution of $\rvv$ and value  of $\sqrt{n}\hat{\boldsymbol{\psi}}$ into $\mathbb{R}$.  This distribution and value in $\mathbb{R}$ can be used to  calculate a p-value. 
% \textcolor{blue}{I don't understand the end of this sentence}. 
This method for finding $p$-values can be used with more complicated functions. We can think of the above test as a function of the test statistic, $\sqrt{n}\hat{\boldsymbol{\psi}}$ and $\sqrt{n}\hat{\boldsymbol{\psi}}$'s limiting distribution.  For the purposes of a test, the distribution used to define any function will be the limiting distribution of $\sqrt{n}\hat{\boldsymbol{\psi}}$ which is distributed according to a multivariate normal with a mean of zero. Because the variance covariance matrix of this multivariate normal (denoted $\Sigma$) \textcolor{blue}{is one-to-one with the possible distributions}, we will index our functions $\Gamma$ by $\Sigma$. 
...
\begin{align}
	\Gamma_\vmat(x) &= Pr_\distv(\|\tilde\rvv + x\| > c_{0.8})  \text{ where }  c_{\vmat, 0.8} \equiv \min_{c}\{c : Pr_\distv(\|\tilde\rvv\| < c) \geq 0.8 \} \text{ and } \tilde\rvv \overset{d}{=} \rvv. \label{gamma:pow}
\end{align}
