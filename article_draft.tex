...
Consider a local alternative in which the true value of $\psi$ is shrinking towards zero at a root $n$ rate: $\psi = \underline{h}/\sqrt{n}$.  For each potential norm, we assume the norm is convex.  This assumption can be relaxed to what was described by Eaton and Perlman (1991) (Concentration inequalitites for multivariate distributions: mv normal).  Under this local alternative, we will have $\sqrt{n} \hat{\psi} \xrightarrow{d} N(\underline{h}, \Sigma)$.    
...
\begin{align*}
	Pr_{\disto}\left(\Gamma^*_{\hat{\distv}}(\sqrt{n}\hat{\boldsymbol{\psi}}) \leq F^{-1}_{\Gamma^*_{\hat{\distv}}(\hat{\distv})}(\alpha)\right) \geq \alpha
\end{align*}
Here (unlike other parts of the paper) small values of $\Gamma$ provide evidence against the null.  Values of $\Gamma$ can be thought of as similar to $p$-valuesx

\begin{lemma}
	\begin{align*}
	\Gamma^*_{\hat{\distv}}(\sqrt{n}\hat{\boldsymbol{\psi}})	\xrightarrow{d}\Gamma^*_{\distv}(\distv) \text{ and } F^{-1}_{\Gamma^*_{\hat{\distv}}(\hat{\distv})}(\alpha) \rightarrow  F^{-1}_{\Gamma^*_{\distv}(\distv)}(\alpha)
	\end{align*}
\end{lemma}
\textcolor{blue}{
...
\begin{align*}
	g(\theta) = \int f_1(\theta) f_2(x - \theta)
\end{align*}
...
For now, we will assume that each of our $\Gamma_Q(x)$ is a unimodal, centrally symmetric function.  In certain cases, $\Gamma_Q(x)$ is increasing $x$ moves away from the origin. In these cases, for the purposes of this proof, instead use $\Gamma^{-1}_Q(x) = \left(\Gamma_Q(x)\right)^{-1}$.  

\begin{lemma}
	\label{lemma:min_cent_sym_unm}
	Let $\Gamma_1(x), \dots, \Gamma_d(x)$ all be centrally symmetric, unimodal functions.  Then $\Gamma^*(x) = \min\left\{\Gamma_1(x), \dots, \Gamma_d(x)\right\}$ is also centrally symmetric and unimodal.
...
\begin{align*}
	\int I\left\{\Gamma^*_Q(x) \leq c_{0.95}\right\} \phi(x - \mu) dx &= \int \left(1 - I\left\{\Gamma^*_Q(x) \geq c_{0.95}\right\}\right)\phi(x - \mu) dx
	\\
