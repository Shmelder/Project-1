...
It follows that if $\mu_n \rightarrow \mu$, and $\Sigma_n \rightarrow \Sigma$, then for each $x$, $\psi(x, \mu_n, \Sigma_n) \rightarrow \psi(x, \mu, \Sigma)$.
This result and the dominated convergence theorem imply the corresponding CDF's are also continuous ($\phi(x, \hat{\mu}, \hat{\Sigma}) + \phi(x, \mu, \Sigma)$ can be used as the dominating measure):
...
\begin{align*}
	\Gamma_{\hat{\Sigma}}(x) - \Gamma_{\Sigma}(x) &= Pr_{\hat{\distv}}(||\rvv^* + x|| > \Phi^{-1}_{\hat{\Sigma}, 0}(0.8)) - Pr_{\distv}(||\rvv^* + x|| > \Phi^{-1}_{\Sigma, 0}(0.8))\\
