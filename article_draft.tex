...
Above we used a norm to constrain the considered rejection regions and allow for a test based on the value of $\sqrt{n}\hat{\boldsymbol{\psi}}$ under the norm and the distribution of $\distv$ under the norm.  However, this approach also works with more complicated functions.  Let $\Gamma_{\distv}(x)$ be a function that maps from a value and distribution in $\mathbb{R}^d$ to $\mathbb{R}$.  The distribution of $\Gamma_\distv(\rvv)$ can be compared to $\Gamma_\distv(\sqrt{n}\hat{\boldsymbol{\psi}})$ to obtain p-values and define a test. An example of $\Gamma$ is given below:
