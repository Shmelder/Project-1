...
\begin{align*}
	Pr_{\disto}\left(\Gamma^*_{\hat{\distv}}(\sqrt{n}\hat{\boldsymbol{\psi}}) \leq F^{-1}_{\Gamma^*_{\hat{\distv}}(\hat{\rvv})}(\alpha)\right) \geq \alpha,
\end{align*}
...
For fixed every $t$,  
\begin{align*}
\Gamma^*_{\hat{\distv}}(t) \rightarrow \Gamma^*_{\distv}(t)	\text{ if } \Gamma_{i, \hat{\distv}}(t) \rightarrow \Gamma^*_{i, \distv}(t) \text{ for every } i \in \{1, 2, \dots, d\}
\end{align*}
